...
- **Mining (TAMBANG)**: 4,797 ESDM IUP concessions vendored to `public/data/idn-tambang.geojson` (`scripts/build-tambang.mjs`), polygon layer coloured by commodity. ESDM `gis1/gis3/gis5` are open+CORS+WGS84 GeoJSON (no token; `gis2`/MOMI/`/arcgis/` are walled — see `docs/research/2026-06-27-civic-data-sources.md`). Paginate at `maxRecordCount=2000`; thin with `maxAllowableOffset`.
- **Kabupaten dossier**: `idn-wilayah.json` (514 kab, ibukota/penduduk/luas, keyless from cahyadsn) drives the **expand-in-place** sub-section; keyed `${prov}|${stripped nama}`. Base geojson had a corrupt 4-vertex sliver feature (now 514, the correct count) — re-check feature counts after any geojson swap.
- **Climate TRACE gotcha**: v6 moved asset coords **`bbox` → `Centroid.Geometry`**; reading the stale field silently fell back to contoh. Layer is now **NON-power** (manufacturing/extraction/fossil-ops — power was the coal plants already in `batubara`, so it overlapped), **top 200 vendored** to `idn-emisi.geojson` via `scripts/build-emisi.mjs`. Fragile schema — re-verify on breakage.
- **Satwa (threatened species)**: 151 IUCN range polygons from **Mandum Rimba** (`mandumrimba.org/data/species-distribution.geojson`, GBIF+IUCN+ESA), browser-direct (CORS *), lazy-loaded on toggle, contoh fallback. MapLibre stringifies the `species` array on click → `JSON.parse` it. Don't re-host (their data is IUCN-derived) — fetch live + credit.
- **Concessions DONE** (2026-07-01): forest+plantation concessions wired via the **KLHK original on BIG SatuPeta** (`build-konsesi.mjs`, 1,040 polys, server-side generalized) — NOT GFW, whose IDN slice is carved out of its CC-BY (research H). **SPPG/MBG kitchens DONE**: ~5,598 from `sismonbgn.com` (`build-sppg.mjs`), honest "28 operating / 5,598". Both ledgers current.
- **Pivots still queued**: Maus 2022 PANGAEA mining footprints (optional, CC-BY) + BIG peatland; OSM hospitals; multi-hazard PetaBencana; **BPS free key** → live IPM/poverty/PDRB in dossiers; optional ESDM power-fleet (3,588) energy-mix layer. And the **newsroom→page wiring gaps** (agenda/makro/harga emitted nowhere, dead verbs) — see PLAN_LOG Track 4. Note `GradienKeadilan` (justice-gradient viz) already exists — dedupe against it.
- **Satwa DONE** (2026-06-28): Mandum Rimba threatened-species ranges wired browser-direct (see gotcha above). Two ledgers (`sumber.astro` + `DATA_SOURCES.md`) brought current with all shipped map layers this session.
