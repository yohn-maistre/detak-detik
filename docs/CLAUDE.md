...
- **Climate TRACE gotcha**: v6 moved asset coords **`bbox` → `Centroid.Geometry`**; reading the stale field silently fell back to contoh. Layer now pulls 4 sectors (power/manufacturing/extraction/fossil-ops), top 250 by tonnage. Fragile schema — re-verify on breakage.
- **Pivots still queued**: GFW deforestation + BIG peatland (forest, R3), OSM hospitals, multi-hazard PetaBencana, `/geo/sppg` kitchens; **BPS free key** → live IPM/poverty/PDRB in dossiers; optional ESDM power-fleet (3,588) energy-mix layer. And the **newsroom→page wiring gaps** (agenda/makro/harga emitted nowhere, dead verbs) — see PLAN_LOG Track 4. Note `GradienKeadilan` (justice-gradient viz) already exists — dedupe against it.
