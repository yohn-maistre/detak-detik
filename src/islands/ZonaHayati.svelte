<script lang="ts">
  ...
  type Row = (typeof HAYATI)[number];
  // the profile plate: the reader's pick while it belongs to this realm,
  // else the day's species here, else the most-threatened one
  let pilihan = $state<Row | null>(null);
  const sorot = $derived.by(() => {
    if (pilihan && (pilihan as { zona?: Zona }).zona === aktif) return pilihan;
    return (hayatiHari as { zona?: Zona }).zona === aktif ? hayatiHari : (daftar[0] ?? hayatiHari);
  });
  let img = $state('');
  ...
  // on hover); click or tap on any row swaps the PROFILE PLATE in place —
  // no popup, the page itself answers (Yose 2026-07-13).
  ...
  // ── the profile prose ── the registry's reviewed line prints first;
  // id.wikipedia may only LENGTHEN it live (Lane A: verbatim, linked)
  let profilTeks = $state('');
  let profilLive = $state(false);
  let sorotEl: HTMLElement | undefined = $state();
  $effect(() => {
    const r = sorot;
    if (!r) return;
    profilTeks = (r as { ringkas?: string }).ringkas ?? '';
    profilLive = false;
    const id = r.id;
    void (async () => {
      try {
        ...
        if (sorot?.id === id && plain.length > profilTeks.length) {
          profilTeks = potong(plain, 1400);
          profilLive = true;
        }
      } catch { /* the reviewed registry text stands */ }
    })();
  });
  function pilihSpesies(r: Row) {
    tip = null;
    pilihan = r;
    // stacked phones: bring the plate into view so the tap visibly answers
    if (window.matchMedia('(max-width: 760px)').matches)
      sorotEl?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }
...
<section class="zh" bind:this={wrap} data-no-stempel data-ref="zona-hayati">
  ...
  <div class="zh-body">
    <!-- the profile plate: a row click swaps it in place, words and all -->
    <figure class="zh-sorot" bind:this={sorotEl}>
      ...
      <figcaption class="zh-sorot-cap">
        <span class="zh-sorot-kicker mono">PROFIL SATWA{sorot && zonaNama(sorot) ? ` · ${zonaNama(sorot).toUpperCase()}` : ''}</span>
        <b class="zh-sorot-nama display">{sorot?.nama}</b>
        <i class="zh-sorot-ilmiah">{sorot?.ilmiah}</i>
        <dl class="zh-sorot-meta mono">
          <div><dt>STATUS IUCN</dt><dd><span class="zh-badge sm mono" data-k={sorot?.status.kode}>{sorot?.status.kode}</span> {(LABEL_RISIKO[sorot?.status.kode ?? ''] ?? sorot?.status.label ?? '').toUpperCase()}</dd></div>
          <div><dt>SEBARAN</dt><dd>{sorot?.wilayah}</dd></div>
          <div><dt>KEDUDUKAN</dt><dd>{sorot?.endemik ? 'ENDEMIK NUSANTARA' : 'IKON KAWASAN'}</dd></div>
        </dl>
        {#if sorot?.endemik}<p class="zh-sorot-endemik fig">{sorot.endemik}</p>{/if}
        {#if profilTeks}<p class="zh-sorot-teks">{profilTeks}</p>{/if}
        <div class="zh-sorot-chips">
          {#if sorot?.wikipedia?.url}
            <a class="chip" href={sorot.wikipedia.url} target="_blank" rel="noopener">⊙ id.wikipedia{profilLive ? ' · langsung' : ' · arsip redaksi'}</a>
          {/if}
          {#if sorot?.status.sumber}<span class="chip" data-no-link>⊙ status · {sorot.status.sumber}</span>{/if}
        </div>
      </figcaption>
    </figure>
    ...
    <div class="zh-list">
          ...
          {#each daftar as r (r.id)}
            <li>
              <button
                ...
                onclick={() => pilihSpesies(r)}
                aria-label={`Tampilkan profil ${r.nama}`}
              >
                ...
                <span class="zh-row-buka mono" aria-hidden="true">PROFIL →</span>
              </button>
            </li>
          {/each}
        </ol>
      ...
      {/if}
      <p class="zh-foot mono">KLIK SEBUAH BARIS — PROFILNYA BERGANTI DI PLAT. URUTAN RISIKO IUCN: CR KRITIS · EN GENTING · VU RENTAN · NT HAMPIR TERANCAM · LC RISIKO RENDAH · NE BELUM DINILAI, DICETAK APA ADANYA. ⊙ gbif + iucn red list · id.wikipedia</p>
    </div>
  </div>
</section>
...
{#if tip}
  <div class="zh-tip" style={`transform: translate(${tip.x}px, ${tip.y - 16}px)`} aria-hidden="true">
    ...
    <div class="zh-tip-teks">
      ...
      {/if}
      <span class="zh-tip-buka mono">↗ KLIK — PROFIL BERGANTI DI PLAT</span>
    </div>
  </div>
{/if}
...
<style>
  ...
  .zh-sorot { margin: 0; display: grid; gap: 10px; }
  .zh-sorot-img { aspect-ratio: 4 / 3; overflow: hidden; border: 1px solid var(--line); background: #ece1c9; }
  .zh-sorot-img img { width: 100%; height: 100%; object-fit: cover; display: block; filter: saturate(0.95); animation: zh-ganti 0.45s var(--ease-out); }
  @keyframes zh-ganti { from { opacity: 0.15; } to { opacity: 1; } }
  @media (prefers-reduced-motion: reduce) { .zh-sorot-img img { animation: none; } }
  .zh-sorot-plat { width: 100%; height: 100%; display: grid; place-items: center; font-size: 9px; letter-spacing: 0.16em; color: var(--muted); }
  .zh-sorot-cap { display: grid; gap: 6px; }
  .zh-sorot-kicker { font-size: 8.5px; letter-spacing: 0.2em; color: var(--accent); }
  .zh-sorot-nama { font-family: 'Fraunces Variable', serif; font-weight: 340; font-size: clamp(24px, 3.4vw, 36px); line-height: 0.98; color: var(--ink); }
  .zh-sorot-ilmiah { font-family: var(--font-fig); font-style: italic; font-size: 12.5px; color: var(--muted); }
  .zh-sorot-meta { display: grid; grid-template-columns: repeat(auto-fit, minmax(130px, 1fr)); gap: 8px 16px; margin: 4px 0 0; border-top: 1px solid var(--line-soft); border-bottom: 1px solid var(--line-soft); padding: 8px 0; }
  .zh-sorot-meta div { display: grid; gap: 3px; }
  .zh-sorot-meta dt { font-size: 7.5px; letter-spacing: 0.16em; color: var(--muted); }
  .zh-sorot-meta dd { font-size: 9.5px; letter-spacing: 0.05em; color: var(--ink); margin: 0; }
  .zh-sorot-endemik { font-size: 13.5px; line-height: 1.5; color: var(--ink); border-left: 3px solid var(--accent); padding-left: 10px; margin: 2px 0 0; }
  .zh-sorot-teks { font-size: 13px; line-height: 1.6; color: var(--ink); margin: 2px 0 0; }
  .zh-sorot-chips { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 2px; }
  .zh-sorot-chips .chip { text-decoration: none; }
  .zh-sorot-chips .chip[data-no-link] { cursor: default; }
